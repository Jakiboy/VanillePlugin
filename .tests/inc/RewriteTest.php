...
class RewriteTest extends WordPressTest
{
    /**
     * @access private
     * @return string $rules
    ...
    {
        if ($this->vars) {
            $this->rules = StringifyTest::replaceArray($this->vars, $this->rules);
        }
        return $this->rules;
    }
    ...
    {
        if ( FileTest::exists( $htaccess = ABSPATH . '/.htaccess') ) {
            $date = date('dmY');
            if ( !FileTest::exists( $backup = ABSPATH . '.htaccess.backup') ){
                FileTest::w($backup, FileTest::r($htaccess));
            }
        }
    }
}
