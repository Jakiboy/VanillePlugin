...
use VanillePluginTest\VanilleConfigTest;
use VanillePluginTest\lib\WordPressTest;
...
use VanillePluginTest\inc\ServerTest;

class PluginOptionsTest extends WordPressTest
{
	use VanilleConfigTest;
	...
	{
		return StringifyTest::toObject( $this->getPluginOption($option) );
	}
	...
	{
		if ( !StringifyTest::contains($icon, 'http') ) {
			$icon = "dashicons-{$icon}";
		}
		$prefix = StringifyTest::replace('-', '_', $this->getNameSpace());
	...
	{
		$id = StringifyTest::replace('.js', '', basename($path));
		$id = StringifyTest::replace('.min', '', $id);
	...
	{
		$prefix = StringifyTest::replace('-', '', $this->getNameSpace());
	...
	{
		$id = StringifyTest::replace('.css', '', basename($path));
		$id = StringifyTest::replace('.min', '', $id);
		$path = "{$this->getAsset()}{$path}";
	...
	{
		$protocol = ServerTest::getProtocol();
		$host = ServerTest::get('HTTP_HOST');
		$request = ServerTest::get('REQUEST_URI');
		$url = "{$protocol}{$host}{$request}";
		$current = ($slug) ? $slug : "?page={$this->getNameSpace()}";
		if ( StringifyTest::contains($url, $current) ) {
			return true;
		}
		return false;
	}
	...
	{
		$callable = StringifyTest::replace('/', '\\', $callable);
		if ( StringifyTest::contains($callable, '\\') ) {
			if ( !FileTest::exists( StringifyTest::formatPath(WP_PLUGIN_DIR."{$callable}.php") ) ) {
				return false;
			}
		}
	...
	public function translateVar($string = '', $var = null)
	{
		...
		return sprintf( $this->translateString(StringifyTest::replace($var, '%s', $string)), $var);
	}
	...
	protected static function removePluginCapability($role, $cap)
	{
		...
		$prefix = StringifyTest::replace('-', '_', $plugin->getNameSpace());
	...
	{
		$nonce = PostTest::isSetted('nonce') ? PostTest::get('nonce') : false;
		if ( !$nonce ) {
			$nonce = GetTest::isSetted('nonce') ? GetTest::get('nonce') : false;
		}
	...
	{
		if ( GetTest::isSetted('settings-updated') 
		&& GetTest::get('settings-updated') == 'true' ) {
			return true;
		}
	}
}
