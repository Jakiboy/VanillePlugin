...
use VanillePluginTest\int\UpdaterInterfaceTest;
use VanillePluginTest\int\PluginNameSpaceInterfaceTest;
use VanillePluginTest\inc\ServerTest;

class UpdaterTest extends PluginOptionsTest implements UpdaterInterfaceTest
{
	/**
	 * @access private
	 * @var array $content
	 ...
	 * action : admin_init
	 */
	public function __construct(PluginNameSpaceInterfaceTest $plugin, $hostUrl, $params = [], $forceUnsafe = false)
	{
		// Init plugin config
		...
		if ($forceUnsafe) {
			$this->unsafe = ServerTest::isHttps() ? false : true;
		} else {
			$this->unsafe = true;
		}
