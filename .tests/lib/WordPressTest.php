...
class WordPressTest
{
	...
	{
		if ( !StringifyTest::contains($path, 'http') ){
	...
	{
		return add_option($option, StringifyTest::serialize($value));
	}
	...
	{
		$option = StringifyTest::unserialize(get_option($option, $default));
		return StringifyTest::slashStrip($option);
	}
	...
	{
		if ( StringifyTest::contains($url,'?ver=') ) {
	...
	{
		return isset($plugin) 
		? StringifyTest::formatPath( WP_PLUGIN_DIR . $plugin ) : WP_PLUGIN_DIR;
	}
	...
	{
		return StringifyTest::formatPath( get_stylesheet_directory() );
	}
